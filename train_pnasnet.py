...
from utils_ft import check, check_training, move_error, print_precision_recall
...
image_datasets = {'train': Create_Dataset_From_CSV('train.csv', train=True, fliplr=True, rotate=True,
                                                   ...
                                                   target_size=331, retrieve_paths=True),
  ...
  for epoch in range(num_epochs):
    ...
    for phase in ['train', 'valid']:
      ...
      if phase == 'valid' and epoch_acc > best_acc:
        ...
        old_model = './checkpoint/resnet101_pnas_{:.3f}.t7'.format(best_acc)
        ...
        torch.save(state, './checkpoint/resnet101_pnas_{:.3f}.t7'.format(best_acc))
...
def main():
  ...
  for param in model.cell_9.parameters():
    param.requires_grad = True
  for param in model.cell_10.parameters():
    param.requires_grad = True
  for param in model.cell_11.parameters():
    param.requires_grad = True
